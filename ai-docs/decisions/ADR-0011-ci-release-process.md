...
| Area | Setting |
|---|---|
| `main` ruleset | No deletion, no force push, linear history, PR required (0 approvals, since the maintainer works solo), conversations resolved, squash merge only, required checks: `lint`, `unit`, `i18n`, `licenses`, `app-build`, `integration (pg16)`, `integration (pg18)`, `docs-build`, `actionlint`, `zizmor`, `dco`. No bypass actors. |
...
| Security | CodeQL default setup (Actions, JavaScript/TypeScript), secret scanning with push protection, Dependabot alerts and security updates, private vulnerability reporting. |
